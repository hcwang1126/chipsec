...
"""
Trusted Execuction Technology (TXT) HAL component
...
TXT_PUBLIC_SPACE_ADDRESS = 0xFED30000
TXT_ERROR_RETURN_SUCCESS = 0x00000000
ACM_EARLY_INIT_SUCCESS = 0xC0830000

TXT_ERROR_CLASS_4_MAJOR_4_MINOR_TABLE = {
}

TXT_ERROR_CLASS_4_MAJOR_TABLE = {
    4 : { 'Minor Table' : TXT_ERROR_CLASS_4_MAJOR_4_MINOR_TABLE, 'Dscr' : 'TPM non-volatile storage is unlocked.'}
}

TXT_ERROR_CLASS_CODE_TABLE = {
    4 : { 'Major Table' : TXT_ERROR_CLASS_4_MAJOR_TABLE, 'Dscr' : 'TPM access error.', }
}

TXT_ERROR_CODE_FIELD = {
    'MODULE_TYPE' : {'name' : 'Module Type', 'bit' : 0, 'size' : 4, 'description' : 'BIOS ACM = 0, SINT = 1.'},
    'CLASS' : {'name' : 'Class Code', 'bit' : 4, 'size' : 6, 'description' : '0 - 0x3f = Class code clusters several congeneric erros into a group.'},
    'MAJOR_ERROR' : {'name' : 'Major Error Code', 'bit' : 10, 'size' : 5, 'description' : '0-0x1f = Error Code within current Class Code.'},
    'SOFTWARE_SOURCE' : {'name' : 'Software Source', 'bit' : 15, 'size' : 1, 'description' : '0 = Authenticated Code Module, 1 = MLE.'},
    'MINOR_ERROR' : {'name' : 'Minor Error Code', 'bit' : 16, 'size' : 12, 'description' : 'The Field value depends on Class Code and or Major Error Code.'},
    'ERROR_SOURCE' : {'name' : 'Processor/Software', 'bit' : 30, 'size' : 1, 'description' : '0 = Error condition reporter by processor, 1 = Error condition reported by software'},
    'VALID' : {'name' : 'Valid', 'bit' : 31, 'size' : 1, 'description' : '1 = Valid.'},
}

ACM_STATUS_FIELD = {
    'TXT_ENABLED' : {'name' : 'TXT Enabled', 'bit' : 62, 'size' : 1, 'description' : '0 = enabled, 1 = disabled.'}
}

TXT_PUBLIC_SPACE = {
   'TXT_ERROR_CODE' : {'offset' : 0x30, 'size' : 4, 'field' : TXT_ERROR_CODE_FIELD},
   'ACM_STATUS' : {'offset' : 0xA0, 'size' : 8, 'field' : ACM_STATUS_FIELD},
   'ACM_ERROR_CODE' : {'offset' : 0x328, 'size' : 4, 'field' : None},
}

class TXT(hal_base.HALBase):
    def __init__( self, cs ):
        self.cs = cs
        self.helper = cs.helper
        #self.TPM_BASE = int(self.cs.Cfg.MEMORY_RANGES["TPM"]["address"], 16)

    def _get_field_value ( self, reg, bit, size ):
        field_mask = 0
        for i in range( size ):
                field_mask = (field_mask << 1) | 1
        return (reg >> bit) & field_mask

    def _dump_reg_field ( self, reg, field ):
        for i in sorted(field.items(), key=lambda x:x[1]['bit']):
            value = self._get_field_value (reg, i[1]['bit'], i[1]['size'])
            form = '0' + str(i[1]['size']) + 'b'
            print "     %s: 0b%s, %s" % (i[1]['name'], format(value, form), i[1]['description'])

        return

    def _get_txt_public_space_reg (self, key):
        reg_address = TXT_PUBLIC_SPACE_ADDRESS + TXT_PUBLIC_SPACE[key]['offset']
        return self.helper.read_mmio_reg( reg_address, TXT_PUBLIC_SPACE[key]['size'] )

    def _dump_txt_public_space_reg ( self, key ):
        title = 'TXT ' + key
        print title

        reg_val = self._get_txt_public_space_reg (key)
        print "Value = 0x%s" % format(reg_val, '08X')

        if TXT_PUBLIC_SPACE[key]['field'] is not None:
            self._dump_reg_field(reg_val, TXT_PUBLIC_SPACE[key]['field'])

        return reg_val

    def dump_txt_error_code ( self ):
        error_code = self._get_txt_public_space_reg ( 'TXT_ERROR_CODE' )
        print "TXT error code in public apace is 0x%s" % format(error_code, '08X')

        if TXT_ERROR_RETURN_SUCCESS != error_code:
            error_code_dict = TXT_ERROR_CODE_FIELD
            valid = self._get_field_value (error_code, error_code_dict['VALID']['bit'], error_code_dict['VALID']['size'])
            print "     Valid bit = %d" % valid

            if valid:
                error_class = self._get_field_value (error_code, error_code_dict['CLASS']['bit'], error_code_dict['CLASS']['size'])
                major_error = self._get_field_value (error_code, error_code_dict['MAJOR_ERROR']['bit'], error_code_dict['MAJOR_ERROR']['size'])
                minor_error = self._get_field_value (error_code, error_code_dict['MINOR_ERROR']['bit'], error_code_dict['MINOR_ERROR']['size'])

                class_code_dscr = None
                major_code_dscr = None
                minor_code_dscr = None
                try:
                    class_code = TXT_ERROR_CLASS_CODE_TABLE[error_class]
                    class_code_dscr = class_code['Dscr']
                    major_code = class_code['Major Table'][major_error]
                    major_code_dscr = major_code['Dscr']
                    minor_code = major_code['Minor Table'][minor_error]
                    minor_code_dscr = minor_code['Dscr']
                except KeyError:
                    pass

                print "     Class Code = %d, %s" % (error_class, class_code_dscr)
                print "     Major Code = %d, %s" % (major_error, major_code_dscr)
                print "     Minor Code = %d, %s" % (minor_error, minor_code_dscr)
            else:
                print "No TXT error is reported..."

    def dump_acm_status ( self ):
        self._dump_txt_public_space_reg ( 'ACM_STATUS' )

    def dump_acm_error_code ( self ):
        if self._dump_txt_public_space_reg ( 'ACM_ERROR_CODE' ) == ACM_EARLY_INIT_SUCCESS:
            print "     BIOS ACM has completed TXT early initializaion successfully."
        else:
            print "     BIOS ACM has not completed TXT early initializaion successfully."
